...
const JobCard = ({ job, isOwner, onViewDetails, onApply, onDelete }) => {
  ...
  return (
    <div className="group relative p-6 glassmorphism glassmorphism-hover rounded-2xl flex flex-col justify-between h-full overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-32 h-32 bg-brand-primary/20 rounded-full blur-3xl -mr-10 -mt-10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>

      <div className="relative z-10">
        <div className="flex justify-between items-start mb-4">
          <h3 className="text-xl font-bold text-white leading-tight group-hover:text-brand-primary transition-colors duration-300">{job.title}</h3>
        </div>
        ...
        <p className="text-sm text-slate-300 line-clamp-2 mb-6 leading-relaxed font-light">{job.description}</p>
      ...
      <div className="relative z-10 flex items-center gap-3 pt-5 border-t border-white/10">
        <button 
          onClick={handleView} 
          className="flex-1 py-2.5 px-4 bg-white/5 text-slate-200 font-medium rounded-xl hover:bg-white/10 transition-colors duration-200 border border-white/10 text-sm text-center"
        >
          Details
        </button>
        {onApply && (
