          ...
          {cards.map((card, idx) => (
            ...
            >
              ...
              <div className="relative z-10">
                <div className="bg-white border border-slate-200 rounded-xl p-4 w-16 h-16 flex items-center justify-center shadow-sm mb-8 group-hover:scale-110 transition-transform duration-300">
                  {card.icon}
                </div>
                <h3 className="text-xl font-bold text-slate-900 mb-4 tracking-wide">{card.title}</h3>
                <p className="text-slate-600 text-base leading-relaxed font-light">{card.description}</p>
              </div>
            </div>
          ))}
        </div>
